...
export default function Home() {
  const { events, cmsContent, user } = useContext(AppContext);
  ...
  const handleGenerateQr = (e) => {
    ...
    if (qrInput.trim()) {
      const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(
        window.location.origin + '/client-login?qr=' + qrInput.trim()
      )}`;
  ...
  return (
    <div style={{ 
      ...
      {/* 2. ACTIVE LIVE EVENTS TICKER (If any events are Active) */}
          ...
          {services.map((srv, idx) => {
            ...
            return (
              <div key={idx} className="glass-panel" style={{
                ...
                <h3 style={{ fontSize: '1.25rem', color: '#fff', fontWeight: '600' }}>{srv.title}</h3>
      ...
      <section style={{ padding: '7rem 2rem', maxWidth: '1200px', margin: '0 auto' }}>
        <div className="glass-panel" style={{
          ...
          <div>
            ...
            <form onSubmit={handleGenerateQr} style={{ display: 'flex', gap: '0.75rem' }}>
              <input 
                type="text" 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
            {generatedQr ? (
              <div className="glass-card animate-fade-in" style={{ padding: '2rem', border: '1px solid var(--gold-primary)', textAlign: 'center', backgroundColor: '#0d0d0d' }}>
                <img src={generatedQr} alt="Event QR Code" style={{ width: '200px', height: '200px', borderRadius: '4px' }} />
                <h4 style={{ fontSize: '0.9rem', color: '#fff', marginTop: '1rem', letterSpacing: '1px', textTransform: 'uppercase' }}>
                  {qrInput.toUpperCase()} QR Code
                </h4>
                <button 
                  onClick={() => navigate(`/client-login?qr=${qrInput}`)}
      ...
      <section style={{ padding: '6rem 2rem', backgroundColor: '#090909', position: 'relative' }}>
        {canEdit && (
          <button 
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', justifyContent: 'center' }}>
            {(test.list || []).map((t, idx) => (
              <div key={idx} className="glass-panel" style={{ padding: '3rem 2rem', flex: '1 1 340px', maxWidth: '450px', textAlign: 'left', backgroundColor: 'rgba(13,13,13,0.3)' }}>
                ...
                <div>
                  <h4 style={{ color: '#fff', fontSize: '1rem', fontWeight: '600' }}>{t.author}</h4>
      ...
      <section style={{ padding: '7rem 2rem', maxWidth: '1200px', margin: '0 auto', position: 'relative' }}>
        ...
        <div style={{
          ...
          <form onSubmit={(e) => {
            e.preventDefault();
            alert('Your luxury consultation inquiry has been submitted! Our studio relations team will contact you within 4 hours.');
            ...
            <div className="form-group">
              ...
              <input type="text" className="form-control" required placeholder="Alexander Vance" />
            </div>

            <div className="form-group">
              <label className="form-label">Email</label>
              <input type="email" className="form-control" required placeholder="alex@vancemail.org" />
            </div>
      ...
      <footer style={{
        ...
        <p style={{ marginBottom: '1.5rem' }}>Fifth Avenue, New York, NY • © 2026. All luxury rights reserved.</p>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem' }}>
          <a href="#" style={{ color: 'var(--text-secondary)' }}>Instagram</a>
          <a href="#" style={{ color: 'var(--text-secondary)' }}>Vimeo</a>
          <a href="#" style={{ color: 'var(--text-secondary)' }}>WhatsApp</a>
          <a href="#" style={{ color: 'var(--text-secondary)' }}>Pinterest</a>
        </div>
      </footer>
    </div>
  );
